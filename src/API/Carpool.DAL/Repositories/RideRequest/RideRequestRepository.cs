...
namespace Carpool.DAL.Repositories.RideRequest
{
	public class RideRequestRepository : BaseRepository<Core.Models.RideRequest, Guid>, IRideRequestRepository
	{
		...
		public async Task<Core.Models.RideRequest> GetByIdAsync(Guid id, CancellationToken cancellationToken)
			=> await _context.RideRequests.FirstOrDefaultAsync(x => x.Id == id, cancellationToken).ConfigureAwait(false);

		public async Task<Core.Models.RideRequest> GetByIdAsNoTrackingAsync(Guid id, CancellationToken cancellationToken)
			=> await _context.RideRequests.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id, cancellationToken).ConfigureAwait(false);
