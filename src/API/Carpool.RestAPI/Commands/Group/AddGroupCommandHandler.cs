...
namespace Carpool.RestAPI.Commands.Group
{
    public class AddGroupCommandHandler : AsyncRequestHandler<AddGroupCommand>
    {
        ...
        {
            if (!string.IsNullOrEmpty(request.Code) && await _repository.GroupCodeExists(request.Code).ConfigureAwait(false)) throw new InvalidOperationException("Group code already exists");
