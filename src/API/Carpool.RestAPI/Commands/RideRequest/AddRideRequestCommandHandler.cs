...
namespace Carpool.RestAPI.Commands.RideRequest
{
	public class AddRideRequestCommandHandler : IRequestHandler<AddRideRequestCommand, Core.Models.RideRequest>
	{
		...
		public AddRideRequestCommandHandler(IRideRequestRepository rideRequestRepository,
		...
		public async Task<Core.Models.RideRequest> Handle(AddRideRequestCommand request,
		                                                  CancellationToken cancellationToken)
		{
			...
			if (request.StartingLocationId != null)
			{
				...
				rideRequest.StartingLocation =
					startingLocation ?? throw new NullReferenceException(nameof(startingLocation));
			}
			else
			{
				var startingLocation = await _locationRepository
				                             .GetByCoordsAsync(request.StartingLocation.Longitude,
					                             request.StartingLocation.Latitude,
					                             cancellationToken).ConfigureAwait(false);
			...
			rideRequest.RequesterId =
				request.RequesterId ?? throw new NullReferenceException(nameof(request.RequesterId));
