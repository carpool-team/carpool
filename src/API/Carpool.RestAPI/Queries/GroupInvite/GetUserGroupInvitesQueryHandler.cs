...
{
	public class GetUserGroupInvitesQueryHandler : IRequestHandler<GetUserGroupInvitesQuery, IEnumerable<Core.Models.GroupInvite>>
	{
		...
		public async Task<IEnumerable<Core.Models.GroupInvite>> Handle(GetUserGroupInvitesQuery request,
			...
			=> await _repository.GetUserGroupInvitesByUserIdAsNoTrackingAsync(request.UserId, cancellationToken).ConfigureAwait(false);
	}
}