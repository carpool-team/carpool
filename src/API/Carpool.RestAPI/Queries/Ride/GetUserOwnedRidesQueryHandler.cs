...
namespace Carpool.RestAPI.Queries.Ride
{
	public class GetUserOwnedRidesQueryHandler : IRequestHandler<GetUserOwnedRidesQuery, IEnumerable<Core.Models.Ride>>
	{
		...
		public async Task<IEnumerable<Core.Models.Ride>> Handle(GetUserOwnedRidesQuery request, CancellationToken cancellationToken)
			=> await _repository.GetOwnedRidesByUserIdAsNoTrackingAsync(request.UserId, request.Past, cancellationToken).ConfigureAwait(false);
	}
}